...
function genderMatches(user: User, candidate: User): boolean {
  ...
  const userWantsCandidate =
    user.genderPreference === "anyone" ||
    user.genderPreference === candidate.gender;

  const candidateWantsUser =
    candidate.genderPreference === "anyone" ||
    candidate.genderPreference === user.gender;

  console.log(
    `Match check: ${user.gender}(wants ${user.genderPreference}) vs ${candidate.gender}(wants ${candidate.genderPreference}) => ${userWantsCandidate && candidateWantsUser}`
  );
